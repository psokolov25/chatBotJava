...
- [12. Backlog дальнейшего развития](#12-backlog-дальнейшего-развития)
...
### 7.4 Разбор частого кейса: `connection timed out` + «кракозябры» в логах
Если в логах одновременно видите:
- `CometD handshake failed ... HTTP connect timed out`,
- `Connect Error: connection timed out after 3000 ms: /<ip>:8080`,
- русские сообщения в виде `╨...` / `╤...`,

то это обычно **две независимые проблемы**:

1. **Сетевая недоступность upstream (основная причина сбоя бизнес-операции).**
   - Бот не может установить TCP-соединение с Orchestra/Axioma (`<ip>:8080`) за `3s`.
   - В текущей конфигурации Micronaut client использует `connect-timeout: 3s`, поэтому ошибки проявляются быстро.

2. **Проблема кодировки консоли/лог-коллектора (косметика, но мешает поддержке).**
   - Текст логов формируется на русском, но где-то по пути читается не как UTF-8.

Рекомендуемый порядок диагностики:
1. С хоста/контейнера бота проверьте маршрут и порт:
   - `nc -vz 192.168.0.38 8080`
   - `curl -v --max-time 5 http://192.168.0.38:8080/`
2. Если порт недоступен — проверить firewall/NAT/VPN/маршрутизацию между контуром бота и контуром очереди.
3. Проверить, что в `application.yml` и env задан корректный `base_url` для нужного филиала.
4. Для нестабильных каналов временно увеличить `micronaut.http.services.default.connect-timeout` (например, до `8-10s`) и повторить тест.
5. Для читаемых русских логов запускать JVM/контейнер с UTF-8 (`LANG=C.UTF-8`, `LC_ALL=C.UTF-8`, `-Dfile.encoding=UTF-8`).

Важно: увеличение таймаута не лечит сетевой разрыв — только уменьшает число ложных срабатываний при кратковременных задержках.
...
## 12. Backlog дальнейшего развития
См. `docs/backlog.md`.
