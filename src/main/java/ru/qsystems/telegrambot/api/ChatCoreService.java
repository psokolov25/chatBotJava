...
public class ChatCoreService {
    private static final Logger log = LoggerFactory.getLogger(ChatCoreService.class);
    ...
    private CoreResponse onPathInput(CoreSessionState state, String value) { BranchConfig branch = branchOf(state); ClientPathConfig path = clientPathService.forBranch(branch).orElseThrow(); PathQuestion q = path.questions().get(state.pathQuestionId); if (q == null) throw new IllegalArgumentException("Вопрос пути не найден"); if (value == null || value.isBlank()) throw new IllegalArgumentException("Значение path-input не должно быть пустым"); String key = q.inputKey() != null ? q.inputKey() : q.questionId(); state.pathAnswers.put(key, value); state.pathAnswers.put(q.text(), value); List<ServiceInfo> services = loadVisibleServices(branch); try { PathScriptResult result = pathScriptExecutor.execute(q.script(), q.scriptId(), Map.of("answer", value, "answers", new HashMap<>(state.pathAnswers))); if (result == null) throw new IllegalArgumentException("Скрипт не вернул результат"); CoreResponse scriptResponse = applyScriptResult(state, branch, path, services, result); if (scriptResponse != null) return scriptResponse; throw new IllegalArgumentException("Скрипт не вернул услуги"); } catch (IllegalStateException ex) { log.warn("Path script failed for question {} in branch {}", q.questionId(), branch.branchId(), ex); return toTemporaryIntegrationErrorResponse(state, q); } }
    ...
    private CoreResponse toTemporaryIntegrationErrorResponse(CoreSessionState state, @Nullable PathQuestion question) {
        List<CoreOption> options = question != null && question.inputType() == ru.qsystems.telegrambot.path.PathInputType.OPTION
                ? toPathOptions(question)
                : List.of(new CoreOption("path-input", "Повторить"));
        return new CoreResponse(state.sessionId, state.visitorId, "Сервис временно недоступен. Возможно потеря связи с СУО. Пожалуйста, повторите шаг через несколько секунд.", options, false, null, null);
    }
